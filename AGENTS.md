...
- API: `from SimplerLLM.language import LLM, LLMProvider`. Factory: `LLM.create(provider=LLMProvider.X, model_name=..., api_key=...)`. Provider enum: OPENAI/GEMINI/ANTHROPIC/OLLAMA/OPENROUTER/...
...
- Frontend sends `apiKey` `provider` strings: `openai`, `anthropic`, `google` (maps to GEMINI), `ollama`, `openrouter` (SimplerLLM's built-in OPENROUTER → `https://openrouter.ai/api/v1`), `omniroute` (any OpenAI-compatible endpoint via the installed `openai` SDK — NOT SimplerLLM; base URL from `OMNIROUTE_BASE_URL`, default `https://omnirouters.com/v1`, overridable per-request via `apiKeys.baseUrl`). Ollama uses the `apiKey` field as base URL and no key.
...
6. Use `LLM.create()` for all provider construction; parse real provider strings from frontend (`google` → GEMINI). Exception: `omniroute` uses the `openai` SDK directly (SimplerLLM can't set a custom base URL).
